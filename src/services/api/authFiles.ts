...
export const authFilesApi = {
  ...
  async getAuthPriority(): Promise<Record<string, number>> {
    const data = await apiClient.get<unknown>('/auth-priority');
    if (!data || typeof data !== 'object') return {};
    const record = data as Record<string, unknown>;
    const source = record['auth-priority'] ?? record;

    // Object format: { name: priority }
    if (source && typeof source === 'object' && !Array.isArray(source)) {
      ...
      Object.entries(source as Record<string, unknown>).forEach(([key, value]) => {
        ...
        if (Number.isFinite(priority)) result[name] = priority;
      });
      return result;
    }

    // Array format: [{ name, priority }]
  ...
  updateAuthPriority: (name: string, priority: number | null) =>
    apiClient.patch('/auth-priority', {
      name,
