...
export type AuthFileCardProps = {
  ...
  authPriorityError?: 'unsupported' | null;
  editingPriorityFile?: string | null;
  editingPriorityValue?: number;
  savingPriority?: string | null;
  onPriorityEdit?: (name: string, currentPriority: number) => void;
  onPrioritySave?: (name: string) => void;
  onPriorityCancel?: () => void;
  onPriorityValueChange?: (value: number) => void;
};
...
export function AuthFileCard(props: AuthFileCardProps) {
  ...
  const currentPriority = authPriority?.[file.name] ?? 0;
  const isPrioritySaving = savingPriority === file.name;
      ...
      <div className={styles.fileCardLayout}>
        <div className={styles.fileCardMain}>
          ...
          <div className={styles.cardMeta}>
            ...
            {!isRuntimeOnly && authPriorityError !== 'unsupported' && onPriorityEdit && (
              <div className={styles.priorityRow}>
                <label className={styles.priorityLabel}>{t('common.priority')}</label>
                {editingPriorityFile === file.name ? (
                  <>
                    <input
                      type="number"
                      ...
                      onChange={(e) => {
                        ...
                        onPriorityValueChange?.(Number.isFinite(value) ? value : 0);
                      }}
                    ...
                    <Button
                      variant="primary"
                      size="sm"
                      onClick={() => onPrioritySave?.(file.name)}
                      disabled={isPrioritySaving}
                  ...
                  <span
                    ...
                    onClick={() => onPriorityEdit(file.name, currentPriority)}
